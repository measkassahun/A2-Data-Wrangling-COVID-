...
location_most_deaths<-counties[counties$death==max(counties$deaths),3]
...
# `location` names (the column with COUNTY, STATE).
lowest_case_general<-counties%>%
  filter(date==min(date))%>%
  filter(cases==min(cases))
lowest_in_each_state<-lowest_case_general
...
# What is the mean number of cases in the nation?
mean_cases_states<-mean(states$cases)
# What is the mean number of cases in the counties?
mean_cases_counties<-mean(counties$cases)
# What is the minimum number of cases in the counties?
min_cases_counties<-min(counties$cases)
